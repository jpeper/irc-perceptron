...
def make_structured_prediction(weights, compiled_training_set, i):     
    summation = float('-inf')
    prediction_index = -1
    if compiled_training_set[i].prev_line == 0:
        summation = 0
        prediction_index = 0

    for j in range(compiled_training_set[i].prev_line):
        temp_sum = 0;            
        for feature in range(len(compiled_training_set[i-j].features)):
            temp_sum += weights[feature] * compiled_training_set[i-j].features[feature]
        if temp_sum > summation:
            summation = temp_sum
            prediction_index = i - j
...
def train_perceptron(weights, compiled_training_set):
    ...
    order_list = list(range(len(compiled_training_set)))
    random.shuffle(order_list)
    
    for i in order_list:

        prediction_index = make_structured_prediction(weights, compiled_training_set, i)
        prediction = make_prediction(weights, compiled_training_set[prediction_index].features)
        ...
        if diff != 0:
            for feature in range(0, len(compiled_training_set[0].features)):    
                weights[feature] += compiled_training_set[i].features[feature] - compiled_training_set[prediction_index].features[feature]
...
def calculate_predictions(weights, test_set_file, correct_predictions, compiled_dictionary):
    ...
    for i in range(len(test_set)):
        ...
        prediction_index = make_structured_prediction(weights, test_set, i)

        prediction = make_prediction(weights, test_set[prediction_index].features)     
...
def generate_annotation_file(weights, d_file, a_file, compiled_dictionary):
    ...
    for i in range(len(test_set)):
        prediction_index = make_structured_prediction(weights, test_set, i)
        test_set[i].prediction = make_prediction(weights, test_set[prediction_index].features)     
